...
class WrapperEMCrossValv3:
    ...
    def compute_cv(self):
        ...
        PYs=np.zeros((self.n_crossval,self.PIEM.n_prot))
        ...
        for i in range(self.n_crossval):
            ...
            PYs[i,:]=self.PIEM.P_I_to_P_Y(self.PIEM.P_I_est)
        return PYs;
    ...
    def load_vars(self):
        self.PFo_given_X_db_flatten=np.fromfile(self.classifier_path+"/binary/Probeam/Common/TopNScores.bin", dtype=np.float32);
        self.true_ids_db=np.fromfile(self.classifier_path+"/binary/Probeam/Common/trueIds.bin", dtype=np.uint32);
...
if __name__ == "__main__":
    ...
    WEMV=WrapperEMCrossValv3(path_dataset,oracle=True,oracle_perr=0.5)
    WEMV.compute_cv()
    print(np.mean(WEMV.errs,axis=0))
